...
pub type ExecutionEnvironment = CircomEnvironment<
    ComponentSlice,
    (TagInfo, TagDefinitions, SignalSlice),
...
pub fn environment_shortcut_add_variable(
    ...
    let slice = AExpressionSlice::new_with_route(dimensions, &AExpr::default());
    environment.add_variable(variable_name, (TagInfo::new(), slice));
}

pub fn environment_check_all_components_assigned(
    environment: &ExecutionEnvironment,
) -> Result<(), (MemoryError, Meta)> {
    ...
    for (name, slice) in environment.get_components_ref() {
        for i in 0..MemorySlice::get_number_of_cells(slice) {
            ...
            if component.is_preinitialized() && component.has_unassigned_inputs() {
                return Result::Err((
                    MemoryError::MissingInputs(name.clone()),
                    component.meta.as_ref().unwrap().clone(),
                ));
            }
        }
    }
