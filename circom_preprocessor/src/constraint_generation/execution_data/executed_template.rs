...
use crate::constraint_generation::execution_data::AExpressionSlice;
use crate::constraint_generation::execution_data::TagInfo;
...
pub struct ExecutedTemplate {
    ...
    pub trace_constraints: Vec<usize>,
    pub final_signal_traces: HashMap<String, usize>,
    pub final_components: HashMap<String, usize>,
}
...
impl ExecutedTemplate {
    pub fn new(
        ...
        ExecutedTemplate {
            ...
            final_signal_traces: HashMap::new(),
            final_components: HashMap::new(),
        }
    }
    ...
    pub fn add_trace_constraint(&mut self, constraint: usize) {
    ...
    pub fn assign_signal_trace(&mut self, full_symbol: String, trace: usize) {
        self.final_signal_traces.insert(full_symbol, trace);
    }

    pub fn assign_component(&mut self, full_symbol: String, node_pointer: usize) {
        self.final_components.insert(full_symbol, node_pointer);
    }
    ...
    pub fn insert_in_dag(&mut self, dag: &mut DAG) {
        let parameters = {
            ...
            for (_, data) in self.parameter_instances.clone() {
                ...
                let values_raw = values.iter().map(|v| v.aexpr.clone()).collect();
                for value in as_big_int(values_raw) {
    ...
    pub fn export_to_circuit(self, instances: &mut [TemplateInstance]) -> TemplateInstance {
        ...
        fn build_arguments(parameter_instances: ParameterContext) -> Vec<Argument> {
            ...
            for (name, data) in parameter_instances {
                ...
                let values_raw = value.iter().map(|v| v.aexpr.clone()).collect();
                let argument = Argument { name, lengths: dim, values: as_big_int(values_raw) };
