...
pub const ZERO_HASHES_POSEIDON: [&[u32]; ZERO_HASHES_MAX_DEPTH] = [
    ...
    &[
        1400117053, 1187826170, 1817951944, 1948085560, 137240011, 559940626, 1385390531,
        1299387419,
    ],
    &[
    ...
    &[
        1640633799, 1177227291, 700748107, 1408925025, 1591794622, 1911595972, 1518230536,
        1010769900,
    ],
    &[
    ...
    &[
        1180415187, 2017991054, 308587294, 207009032, 1456059267, 2041063345, 1182897744,
        1709394793,
    ],
    &[
...
fn test_zero_hashes_poseidon() {
    let param = PoseidonParams::new(POSEIDON_M31X16_RATE, 16, 8, 14);
...
pub fn merkleize_var_with_limit<C: Config, B: RootAPI<C>>(
    ...
    for i in 0..depth - 1 {
        if cur_level.len() % 2 == 1 {
            ...
            (0..param.rate)
                .for_each(|j| padding[j] = builder.constant(ZERO_HASHES_POSEIDON[i + 10][j]));
