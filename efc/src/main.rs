...
use efc::end2end::{debug_eval_all_assignments, end2end_prepare_solver, end2end_witnesses_from_beacon_data};
...
fn main() {
    ...
    if let (Some(s_index), Some(e_index), Some(m_index)) = (
        ...
        match (args.get(s_index + 1), args.get(e_index + 1), args.get(m_index + 1)) {
            (Some(stage), Some(epoch_str), Some(mpi_config_str)) => match epoch_str.parse::<u64>() {
                Ok(epoch) => {
                    println!(
                        "Running stage: {} at epoch: {}, with mpi_config: {}",
                        stage, epoch, mpi_config_str
                    );
                    let mpi_config: Vec<usize> = mpi_config_str.split(',').filter_map(|x| x.parse().ok()).collect();
