...
impl Default for AttestationSSZ {
    fn default() -> Self {
        Self {
...
impl AttestationSSZ {
    ...
    pub fn att_data_signing_root<C: Config, B: RootAPI<C>>(&self, builder: &mut B, att_domain: &[Variable]) -> Vec<Variable> {
        let att_data_hash_tree_root = self.hash_tree_root(builder);
        let inputs = vec![att_data_hash_tree_root, att_domain.to_vec()].concat();
        bytes_hash_tree_root(builder, inputs)
    }

    pub fn check_point_hash_tree_variable<C: Config, B: RootAPI<C>>(&self, builder: &mut B, epoch: &[Variable], root: &[Variable]) -> Vec<Variable> {
    ...
    pub fn hash_tree_root<C: Config, B: RootAPI<C>>(&self, builder: &mut B) -> Vec<Variable> {
        ...
        let source_checkpoint_root = self.check_point_hash_tree_variable(builder, &self.source_epoch, &self.source_root);
        inputs.extend_from_slice(&source_checkpoint_root);
        let target_checkpoint_root = self.check_point_hash_tree_variable(builder, &self.target_epoch, &self.target_root);
        inputs.extend_from_slice(&target_checkpoint_root);
        bytes_hash_tree_root(builder, inputs)
    }
}
pub fn bytes_hash_tree_root<C: Config, B: RootAPI<C>>(builder: &mut B, inputs: Vec<Variable>) -> Vec<Variable> {
    let chunks = to_chunks(&append_to_32_bytes(builder, &inputs));
    beacon_merklize(builder, chunks).unwrap()
}
pub fn beacon_merklize<C: Config, B: RootAPI<C>>(builder: &mut B, inputs: Vec<Vec<Variable>>) -> Result<Vec<Variable>, String> {
    if inputs.is_empty() {
    ...
    for i in 0..depth {
        if inputs.len() % 2 == 1 {
            let pad_hash = ZERO_HASHES[i];
            let padding: Vec<_> = pad_hash.iter().map(|&x| builder.constant(x as u32)).collect();
            inputs.push(padding);
        }
...
pub fn append_to_32_bytes<C: Config, B: RootAPI<C>>(builder: &mut B, input: &[Variable]) -> Vec<Variable> {
    if let rest = input.len() % 32 {
        if rest != 0 {
