use expander_compiler::frontend::{BasicAPI, Config, Variable, API};

pub fn u32_to_bits<C: Config>(api: &mut API<C>, value: u32) -> Vec<Variable> {
    return (0..32)
        .map(|x| api.constant(((value >> x) & 1) as u32))
        .collect();
}

pub fn u8_to_bits(value: u8) -> Vec<u8> {
    return (0..8).map(|x| (value >> x) & 1).collect();
}
