...
#[memorized]
fn compute_keccak2<C: Config>(api: &mut API<C>, p: &Vec<Variable>) -> Vec<Variable> {
    let mut ss = vec![vec![api.constant(0); 64]; 25];
    let mut new_p = p.clone();
    let mut append_data = vec![0; 136 - 64];
    append_data[0] = 1;
...
#[memorized]
fn test_aaa<C: Config>(
    api: &mut API<C>,
    a: Vec<Variable>,
    b: &Vec<Variable>,
    c: Variable,
    d: &Variable,
    e: Vec<Vec<Vec<Variable>>>,
    f: &Vec<Vec<Vec<Variable>>>,
    g: Vec<Vec<Vec<usize>>>,
    h: &u64,
    i: &Vec<i32>,
) -> Variable {
...
#[memorized]
fn test_bbb<C: Config>(api: &mut API<C>) {}

#[memorized]
fn test_ccc<C: Config>(api: &mut API<C>) -> Vec<Vec<Vec<Variable>>> {
    panic!("test")
}
