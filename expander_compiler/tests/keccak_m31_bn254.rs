...
use expander_compiler::utils::interpreter_loader::M31Loader;
use internal::Serde;
...
declare_circuit!(KeccakFCircuit {
  p: [Variable; 25],
  q: [Variable; 200],
});

impl<C: Config> Define<C> for KeccakFCircuit<Variable> {
  fn define(&self, api: &mut API<C>) {
    let mut data = self.p.to_vec();
    data.append(&mut self.q.to_vec());
    api.memorized_simple_call(check_keccakf, &data);
  }
}

fn check_keccakf<C: Config, B: RootAPI<C>>(
  api: &mut B,
  origin_data: &Vec<Variable>,
) -> Vec<Variable> {
  let mut loader = M31Loader::new();
  let fname = env!("CARGO_MANIFEST_DIR").to_string() + "/circuits/keccakf_m31.txt";
  let input = vec![origin_data[..25].to_vec()];
  let mut result = vec![];
  loader.load(&fname, &input, &mut result, api);
  let output = origin_data[25..].to_vec();
  for i in 0..200 {
    api.display(&format!("out.{}", i), result[0][i][0].clone());
    api.assert_is_equal(output[i], result[0][i][0]);
  }
  result[0].iter().map(|x| x[0].clone()).collect()
}

#[test]
fn keccak_m31_compiled() {
  let mut q = [0u64; 25];
  tiny_keccak::keccakf(&mut q);
  let compile_result: CompileResult<M31Config> = compile(&KeccakFCircuit::default()).unwrap();
  let CompileResult {
    witness_solver,
    layered_circuit,
  } = compile_result;
  let mut assignment = KeccakFCircuit::default();
  for i in 0..25 {
    assignment.p[i] = M31::from(0);
  }
  for i in 0..25 {
    assignment.q[i * 8 + 0] = M31::from(((q[i] >> (0 * 8)) & 0xff) as u32);
    assignment.q[i * 8 + 1] = M31::from(((q[i] >> (1 * 8)) & 0xff) as u32);
    assignment.q[i * 8 + 2] = M31::from(((q[i] >> (2 * 8)) & 0xff) as u32);
    assignment.q[i * 8 + 3] = M31::from(((q[i] >> (3 * 8)) & 0xff) as u32);
    assignment.q[i * 8 + 4] = M31::from(((q[i] >> (4 * 8)) & 0xff) as u32);
    assignment.q[i * 8 + 5] = M31::from(((q[i] >> (5 * 8)) & 0xff) as u32);
    assignment.q[i * 8 + 6] = M31::from(((q[i] >> (6 * 8)) & 0xff) as u32);
    assignment.q[i * 8 + 7] = M31::from(((q[i] >> (7 * 8)) & 0xff) as u32);
  }

  let mut hint_registry = HintRegistry::<M31>::new();
  hint_registry.register("myhint.tobinary", M31Loader::to_binary_hint);

  // debug_eval(&KeccakFCircuit::default(), &assignment, hint_registry);

  let witness = witness_solver
    .solve_witness_with_hints(&assignment, &mut hint_registry)
    .unwrap();
  let run = layered_circuit.run(&witness);
