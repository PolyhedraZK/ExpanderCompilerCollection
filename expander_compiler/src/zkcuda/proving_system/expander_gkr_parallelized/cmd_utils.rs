...
pub fn start_server<C: GKREngine>(max_parallel_count: usize, ip_addr: &str, port_number: u16)
where
    C::FieldConfig: FieldEngine<SimdCircuitField = C::PCSField>,
{
    ...
    let cmd_str = format!(
        "mpiexec -n {max_parallel_count} {overscribe} ../target/release/expander_serve --field-type {field_name} --poly-commit {pcs_name} --ip-addr {ip_addr} --port-number {port_number}"
    );
