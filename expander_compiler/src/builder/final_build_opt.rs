...
mod tests {
            ...
            super::InCircuit::<C> {
                instructions: vec![ir::hint_less::Instruction::LinComb(ir::expr::LinComb {
                    terms: vec![
                        ...
                        ir::expr::LinCombTerm {
                            coef: CField::from(2u32),
                            var: 2,
                        },
                    ],
                    constant: CField::from(3u32),
                })],
                constraints: vec![3],
                outputs: vec![],
                num_inputs: 2,
        ...
        assert_eq!(
            ...
            ir::dest::Instruction::InternalVariable {
                expr: Expression::from_terms(vec![
                    ...
                    Term::new_linear(CField::from(2u32), 2),
                    Term::new_const(CField::from(3u32))
                ])
            }
        );
    ...
    fn simple_mul() {
        ...
        assert_eq!(out, vec![CField::from(2 * 3 * 5 * 7u32)]);
    }
    ...
    fn large_add() {
        ...
        let inputs: Vec<CField> = (1..=100000u32).map(CField::from).collect();
