...
var (
	poseidonM31x16FullRounds    int
	poseidonM31x16PartialRounds int
...
func init() {
	...
	var m31Modulus uint = uint(m31.ScalarField.Uint64())
...
func poseidonM31x16RoundConstantApply(
	api frontend.API, state []frontend.Variable, round int) []frontend.Variable {
...
func PoseidonM31x16Permutate(
	...
	for i := 0; i < poseidonM31x16FullRounds/2; i++ {
...
func (p *PoseidonM31x16Permutation) Define(api frontend.API) error {

	digest := poseidonM31x16FullRoundSBox(api, p.State[:])

	for i := 0; i < 16; i++ {
		api.AssertIsEqual(p.Digest[i], digest[i])
	}
...
func (p *PoseidonM31x16Sponge) Define(api frontend.API) error {
	digest, _ := PoseidonM31x16HashToState(api, p.ToBeHashed)

	for i := 0; i < 16; i++ {
		api.AssertIsEqual(digest[i], p.Digest[i])
	}
